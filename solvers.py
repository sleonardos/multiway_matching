...
class GradientDescent():
    ...
    def __init__(self, learning_rate = 0.1, max_iter = 1000, tol = 1e-6, pert = 1e-6):
...
class ConjugateGradient():
    ...
    def __init__(self, max_iter = 1000, tol = 1e-6, pert = 1e-6):
    ...
    def LineSearch(self, manifold, cost, X, eta, grad):
        ...
        alpha, sigma, b = 1, 0.5, 0.5
        max_cost_evals = 25
        Y = manifold.retraction(X,alpha*eta)
        costX, costY = cost(X), cost(Y)
        cost_evals = 1
        df = manifold.metric(X, grad, eta)
        while  costY > costX + alpha*sigma*df:
                alpha *= b
                Y = manifold.retraction(X,alpha*eta)
                costY = cost(Y)
                cost_evals+=1
    ...
    def Solve(self, manifold, cost, egrad, X):
        ...
        for i in range(self.max_iter):
            ...
            #beta = manifold.norm(X, grad) / manifold.norm(X_old, grad_old)

            # Polak-Ribiere beta
            beta = manifold.metric(X, grad, grad- manifold.transport(X_old, X, grad_old) ) / manifold.norm(X_old, grad_old)
            beta = max(0,beta)
            ...
            # cache cost and norm of gradient
            costX, grad_norm  = cost(X), manifold.norm(X,grad)
            cache.append((costX,grad_norm))
